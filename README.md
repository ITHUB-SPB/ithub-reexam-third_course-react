# Пересдача дисциплины "Фронтенд-фреймворки" за 3 курс I семестр

## Описание проекта

Приложение представляет игру в крестики-нолики.

Игроки делают ходы по очереди (начинают всегда крестики). Иконка игрока, чей ход ожидается в данный момент, отображаются нормально, а второго игрока - со сниженной насыщенностью. 

Когда один из игроков получает три-в-ряд, эти ячейки (а также иконка игрока) подсвечиваются зеленым. При ничьей подсветки не происходит.

По завершению игры (победой или ничьей) появляется кнопка "Начать заново", при нажатии на которую состояние игры сбрасывается.

## Требования

### Стек

Использовать фреймворк `React 19` и сборщик `Vite 8`. Все необходимые библиотеки уже включены в `package.json`, добавлять что-либо помимо них не стоит.

### Дизайн

Базовый дизайн уже реализован. Для доработок и вдохновения можете пользоваться [макетом](https://www.figma.com/design/1rKJjzNZJ7BL06XaFqk8Xj/Tic-Tac-Toe--Community-?node-id=0-1&p=f&t=2bQ6SV4kGJmD35Qa-0) 

Повторять макет в точности не требуется.

### Компоненты

При реализации придерживайтесь следующей структуры компонентов:

- экран игры
  - легенда
    - иконки игроков
  - игровое поле
    - ячейка игрового поля
      - иконка внутри него
  - кнопка "начать снова"

### Хранение состояния

Состояние игры должно храниться без дополнительных библиотек. Можно использовать лишь следующие встроенные хуки: 

- useState
- useEffect / useLayoutEffect
- useReducer
- useEffect
- useCallback

Состояние игры включает в себя:

- состояние поля (двумерный массив состояний ячеек поля)
- статус (x-wins, o-wins, draw, x-move, o-move)

Поле при первом запуске (либо перезапуске) имеет следующий вид:

```ts
[
    [null, null, null],
    [null, null, null],
    [null, null, null]
]
```

Статус при первом запуске имеет вид

```ts
"x-move"
```

При завершении игры победой крестиков поле может иметь вид

```ts
[
    ["x", "x", "x"],
    [null, "o", null],
    [null, "o", null]
]
```

А статус будет равен 

```ts
"x-wins"
```

## Критерии оценивания

1. При клике по полю игры изменяется состояние игрового поля, и в соответствии с ним обновляется UI (2 балла).

2. Ходы игроков чередуются (1 балл).

3. Корректно определяется статус на поле: победа, ничья, или игра еще в процессе (3 балла).

4. Если один из игроков получает три-в-ряд, его ячейки и его иконка подсвечиваются зеленым (2 балла).

5. По нажатию на кнопку "Начать сначала" происходит сброс состояния игры, и в нее можно сыграть заново (2 балла). Кнопка "Начать сначала" отображается только по завершению игры (1 балл).

Для перевода в 100-балльную систему полученный по данным критериям результат будет **умножен на 7**. Таким образом, для **максимального результата** нужно получить **10 баллов**, а для проходного балла - **7**.